...
.navbar {
  ...
  height: 80px; /* Fixed height to match main page */
}
/* Logo Styling */
.navbar img {
  height: 100%; /* Dynamically scale logo to header height */
  max-height: 100%; /* Prevent overflow */
  margin: 0; /* Remove margin to position the logo flush left */
  object-fit: contain; /* Keep logo proportions */
}
/* Navigation Links */
.navbar nav {
  display: flex;
  justify-content: space-around; /* Spread links evenly across available space */
  flex: 1; /* Allow nav to take up remaining space after logo */
}
.navbar a {
  text-decoration: none;
  color: white;
  font-size: 1.2rem; /* Match link size with the main page */
